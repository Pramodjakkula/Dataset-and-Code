...
def runLSTM():
    ...
    if os.path.exists("model/lstm_model.json"):
        ...
        lstm.add(Dense(output_dim = y_train.shape[1], activation = 'softmax'))
        #compile the model
        lstm.compile(optimizer = 'adam', loss = 'categorical_crossentropy', metrics = ['accuracy'])
        #start training model
        hist = lstm.fit(X_train, y_train, batch_size=16, epochs=10, shuffle=True, verbose=2, validation_data=(X_test, y_test))
        ...
        f.close()
    print(lstm.summary())
    predict = lstm.predict(X_test) #perform predition on test data
    predict = np.argmax(predict, axis=1)
    y_test = np.argmax(y_test, axis=1)
    lstm_mse = 1.0 - accuracy_score(y_test,predict) #calculate MSE value
    mse.append(lstm_mse)
